...
## 10. 关于密钥与部署：是否用 FastAPI + 前端静态资源？

结论：**是，推荐。**

如果你关注的是“密钥不泄露 + 服务端统一编排 + 后续可扩展”，一个非常稳妥的形态是：

- 前端（React/Vite）只负责 UI，`npm run build` 产出静态文件（`dist/`）
- 后端（FastAPI）负责：
  - RSS 抓取与排序
  - LLM 调用（Gemini/Ollama/OpenAI）
  - Finnhub 调用
  - 健康检查、审计日志、限流、缓存
  - 提供 `/api/*` 接口给前端
- 生产环境由 Nginx/Caddy 统一反代：
  - `/` -> 前端静态资源
  - `/api/*` -> FastAPI

### 为什么这比“前端直连模型”更好

1. **密钥边界正确**：所有第三方 API Key 只存在服务端环境变量。
2. **可观测性更强**：可在服务端记录 token 成本、失败率、响应时延。
3. **策略统一**：去重、评分、置信度、重试逻辑都可集中管理。
4. **多模型切换容易**：FastAPI 内部做 provider 抽象，前端无需改动。

### 是否必须由 FastAPI 直接托管静态文件？

有两种都可行：

1. **Nginx 托管静态 + 反代 FastAPI（推荐生产）**
   - 更高性能、缓存策略更灵活、部署更标准。
2. **FastAPI 直接挂载静态目录（推荐本地/轻量）**
   - 单进程更简单，但大流量下不如 Nginx 专业。

### 最小落地建议

Week 1 你可以先做这 4 步：

1. 把 Gemini 调用从前端完全迁到后端（前端不再读取任何模型密钥）。
2. 定义统一接口：
   - `POST /api/news/generate`
   - `POST /api/market/generate`
   - `GET /api/health?provider=gemini|ollama|finnhub`
3. 前端只调用上述接口，provider 作为参数传入。
4. 先用 FastAPI `StaticFiles` 挂载 `dist/`，跑通后再接 Nginx。
