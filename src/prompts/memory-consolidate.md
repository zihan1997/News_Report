...
- Do not return an empty array to mean "keep the existing section." If a section remains useful, return the cleaned existing bullets.
- Return an empty array only when the current memory truly has no useful content for that section and clearing it is intentional.
...
- "企业攻击面已从传统网络边界扩展到身份、协作工具和供应链等高权限入口。"
- "多起独立安全事件共享同一攻击面主题，企业防护重点需要从边界防护扩展到连续治理。"

Bad story resolved examples:
- "KongTuke组织使用Microsoft Teams进行企业渗透，最短5分钟获得持久访问。"
- "Cisco Catalyst SD-WAN Controller认证绕过漏洞CVSS 10.0被积极利用。"
- "Microsoft Exchange Server零日漏洞CVE-2026-42897被积极利用，CVSS 8.1。"
- "支付供应链漏洞表明电商环节已成为企业攻击面的新延伸。"

Good story currentState example:
- "企业AI时代的网络攻击面正在从传统边界扩展到身份、协作工具和供应链等高权限入口。近期多起事件显示，攻击者正在围绕企业日常工作流和关键基础设施寻找更快、更高权限的进入点。当前主线不是某个单一漏洞，而是企业防护重心需要从边界安全转向连续治理。"
- "美联储领导层完成过渡后，政策路径不确定性显著上升。长端收益率上行和通胀压力共同强化了市场对higher-for-longer的担忧，使Fed政策预期成为影响科技估值、风险偏好和市场广度的核心宏观背景。"
...
Narrative section semantics:
- currentState: the broad cross-story thesis, not a single story recap.
- weakSignals: caveats, contrary evidence, fragile assumptions, or places where the thesis may be overextended.
- openQuestions: unresolved analytical questions that would strengthen or weaken the narrative.
- watchlist: concrete signals to monitor next.
- Do not let a narrative collapse into one story. If only one story supports it, say the narrative remains weak.
- Keep all narrative arrays concise and useful. Prefer preserving existing good bullets over returning blank arrays.
